...
const ResRoom = {
    ...
    selectForInvoice: (id, cb) => {
        const queryString = "SELECT rr.res_room_id, ((DATE(rr.check_out_date)-DATE(rr.check_in_date)) * rr.rate) AS room_total, (SELECT ((t.county_tax_rate+t.city_tax_rate+t.state_tax_rate)/100) AS tm FROM tax_rates AS t WHERE t.tax_rate_id=1) AS tax_multiplier FROM res_rooms AS rr WHERE rr.res_room_id=? LIMIT 1";
