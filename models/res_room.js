...
const ResRoom = {
    selectForHouseStatus: (date, cb) => {
        ...
        const queryString = "SELECT COALESCE(SUM(CASE WHEN rr.check_in_date<@input_date && rr.check_out_date>@input_date THEN 1 ELSE 0 END), 0) AS stayovers, COALESCE(SUM(CASE WHEN rr.check_out_date=@input_date && rr.checked_out=0 THEN 1 ELSE 0 END), 0) AS departuresPending, COALESCE(SUM(CASE WHEN rr.check_out_date=@input_date && rr.checked_out=1 THEN 1 ELSE 0 END), 0) AS departuresActual, COALESCE(SUM(CASE WHEN rr.check_in_date=@input_date && rr.checked_in=0 THEN 1 ELSE 0 END), 0) AS arrivalsPending, COALESCE(SUM(CASE WHEN rr.check_in_date=@input_date && rr.checked_in=1 THEN 1 ELSE 0 END), 0) AS arrivalsActual FROM res_rooms AS rr WHERE rr.active=1 && (rr.check_in_date<@input_date && rr.check_out_date>@input_date) || rr.check_out_date=@input_date || rr.check_in_date=@input_date;";
    ...
    countPendingDeparturesByRoomType: (date, cb) => {
        const queryString = "SELECT rt.room_type_id, rt.type, COUNT(rr.room_type_id) AS pending_departures FROM res_rooms AS rr INNER JOIN room_types AS rt ON rr.room_type_id=rt.room_type_id WHERE rr.active=1 && rr.checked_out=0 && rr.check_out_date=? GROUP BY rr.room_type_id ORDER BY rt.room_type_id ASC;";
